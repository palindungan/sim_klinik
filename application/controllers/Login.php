...
class Login extends CI_Controller
{
    ...
    function cek2(){
        ...
        while ($date_temp < $endTimeStamp) { //Loop PerHari, $date_temp dimulai dari tanggal terakhir yang tersimpan dalam Tabel Temp Saldo
            ...
            //Laporan RI Pelayanan
            $laporan_ri_harian_new_pelayanan = $this->M_laporan->laporan_ri_harian_new_pelayanan($next_day);
            //Loop:
            foreach($laporan_ri_harian_new_pelayanan as $row){
                // Validasi Value Karena Bukan Tipe Integer
                $uang_masuk = (int) $row->uang_masuk;
                $gizi = (int) $row->gizi;
                $gda = (int) $row->gda;
            ...
            //Laporan RI AKOMODASI
            $laporan_ri_harian_new_akomodasi_ri = $this->M_laporan->laporan_ri_harian_new_akomodasi_ri($next_day);
            //Loop:
            foreach($laporan_ri_harian_new_akomodasi_ri as $row){
            ...
            $laporan_ri_harian_new_setoran_ri = $this->M_laporan->laporan_ri_harian_new_setoran_ri($next_day);
            //Loop:
